...
internal class GridViewAdapter (
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View? {
        ...
        if (convertView == null) {
            convertView = layoutInflater!!.inflate(R.layout.plant_item, null)
        }
        ...
        des = convertView!!.findViewById(R.id.txtDes)

        name.setText(courseList.get(position).name)
        owner.setText(courseList.get(position).owner)
        date.setText(courseList.get(position).date)
        des.setText(courseList.get(position).description)
